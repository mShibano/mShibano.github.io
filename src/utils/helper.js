...
function isItClose(player, targets) {
...
function interact(message, player, objs, sound) {
    if (objs === void 0) { objs = []; }
    ...
    if (message.text) {
        ...
        var nextToTarget = isItClose(player, objs);
        if (nextToTarget) {
            console.log("im next to: ".concat(nextToTarget.name));
